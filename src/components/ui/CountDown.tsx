...
function YearCountdown() {
  ...
  const renderer = ({
    ...
    return (
      <div>
        {/* <div>{remainingYears} years</div>
        <div>{remainingMonths} months</div>
        ...
        <div className="grid auto-cols-max grid-flow-col text-center md:gap-5">
          {remainingYears > 0 && (
            <div className="rounded-box flex flex-col bg-primary p-2 text-neutral-content">
          ...
          <div className="rounded-box flex flex-col p-2 text-neutral-content md:bg-primary">
            <span className="countdown font-mono md:md:text-5xl">
              <span
                style={{ "--value": remainingDays } as CSSProperties}
              ></span>
            </span>
            days
          </div>
  ...
  return (
    <div>
      <Countdown date={data.dateTime} renderer={renderer} />
    </div>
  );
}
